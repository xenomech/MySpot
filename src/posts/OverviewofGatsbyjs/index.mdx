...
SSG stands for Static Site Generator. An SSG takes in templates, components, data and provides you with a static HTML page. Static site generators are an alternative to database-driven content management.
...
2. **The gatsby-cli**
   ...
   The Gatsby CLI tool lets you run gatsby commands on the command line. It is an npm package that we need to install.

   ```sh
   npm install -g gatsby-cli
   ```

   which will install gatsby-cli globally in your system.
...
If you have a component, _about.js_ inside the pages folder.
...
[GraphQL](https://graphql.org/) is a query language developed by Facebook. It interacts with APIs. Queries allow you to get all the information you need inside of a single request.
Gatsby uses Graphql to interact with local files. This allows you to reuse pieces of data.
...
- _gatsby-node.js_ is executed once in the process of building your site.
  You can use it to dynamically create pages, add Graphql Nodes, etc.
...
- _gatsby-ssr.js_ Related with Server-Side-Rendering. This file will let you alter the content of static HTML files while they are being rendered by the server.
