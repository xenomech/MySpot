...
date: 2021-05-05
excerpt: Exploring the use-case of the curved_navigation_bar package in Flutter
...
A navigation bar is a section of a GUI intended to provide users with the ability to switch between different slices of the app with a single tap. The
...
Go ahead and clear all the boilerplate code in the _lib/main.dart_ and make a _lib/pages/index.dart_.
...
Make a stateful widget called "IndexPage" in the index.dart file.
...
![The index page](./Indexpage.png)

The next step is to add the curved*navigation_bar package into the \_pubspec.yaml* file.
...
Imagine we have five pages that the navbar.
...
Let's create these pages, as this is just a demonstration we'll make all the pages as stateless widgets.

Before we start making the pages we must be able to preview these pages as we make them. So, to do that we must edit **lib/pages/index.dart** .
...
Now let's edit the index page so that we can access all these pages using the navbar.
...
Next, declare a list with all the declared widgets in the order in which you want to show them, because we will be using the index to select them.
...
Now use the curved_navigation_bar as the bottomNavigationBar property.
...
Now we need to provide the attributes which the Curved navbar uses.

Such as items,index,color,backgroundColor,onTap,etc

Now, Lets add the list od items we had
...
The widget corresponding to tabs[_activePage] can be provided as the body property for the scaffold.
...
```
This will set the _activePage to the index of the item tapped.

Finally,bringing everything together, we have
...
<img src="https://github.com/xenomech/MySpot/blob/master/src/posts/The%20Curved%20Navigation%20in%20Flutter/untitled.gif?raw=true" alt="Output" width="200"/>
